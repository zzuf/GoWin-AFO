...
| Windows SDK contract/union WinMD | The pinned [`Microsoft.Windows.SDK.CPP/10.0.26100.7705`](https://www.nuget.org/packages/Microsoft.Windows.SDK.CPP/10.0.26100.7705) package's nuspec requires license acceptance and links to the [Windows SDK license](https://aka.ms/WinSDKLicenseURL). It does not contain an embedded SDK license text; its C++/WinRT `LICENSE.txt` is not the SDK license. | Fetch the exact archive, verify its SHA-256, and cache only the selected UnionMetadata WinMD and provenance nuspec alongside the verified archive. Do not vendor or attach the package or WinMD to releases. The WinMD bytes are unchanged from the former installed-SDK input. |
